...
public sealed class PropelloTemplateMemoryRepository : IPropelloTemplateMemoryRepository
{
	private const string PackagedAssetName = "propello_template_memory.json";
	...
	public async Task<PropelloTemplateMemoryDocument> LoadAsync(CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			var document = await JsonSerializer.DeserializeAsync<PropelloTemplateMemoryDocument>(stream, SerializerOptions, cancellationToken);
			return document ?? new PropelloTemplateMemoryDocument();
		}
		finally
		{
			gate.Release();
	...
	public async Task SaveAsync(PropelloTemplateMemoryDocument document, CancellationToken cancellationToken = default)
	{
	...
	private static async Task<Stream> OpenPackagedAssetAsync(CancellationToken cancellationToken)
	{
		...
		throw new FileNotFoundException(
			$"Unable to locate the packaged Propello template memory asset. Tried: {string.Join(", ", PackagedAssetCandidates)}.");
	}
}