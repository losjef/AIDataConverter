...
public sealed class SemanticDictionaryRepository : ISemanticDictionaryRepository
{
	private const string PackagedAssetName = "semantic_dictionary.json";
	...
	public async Task<SemanticDictionaryDocument> LoadAsync(CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			var document = await JsonSerializer.DeserializeAsync<SemanticDictionaryDocument>(stream, SerializerOptions, cancellationToken);
			return document ?? new SemanticDictionaryDocument();
		}
		finally
		{
			gate.Release();
	...
	public async Task SaveAsync(SemanticDictionaryDocument document, CancellationToken cancellationToken = default)
	{
	...
	private static async Task<Stream> OpenPackagedAssetAsync(CancellationToken cancellationToken)
	{
		...
		throw new FileNotFoundException(
			$"Unable to locate the packaged semantic dictionary asset. Tried: {string.Join(", ", PackagedAssetCandidates)}.");
	}
}